...
arxiv_meta_db="/CurrentPorjects/LatentDirichletAllocation/liblda/topicviz/tmve_ajbchaney/arXiv_demo/arXiv_metadata.db"
...
def get_doc_display(doc):
	...
	# assume doc is a string like 9412002v1
	con = sqlite3.connect(arxiv_meta_db)
	con.row_factory = sqlite3.Row
	cur = con.cursor()
	
	# strip only the number (no version info)
	arxiv_num = doc.title[0:7]
	
	cur.execute('SELECT *  FROM papers WHERE arxiv_num=?', [arxiv_num])
	metadata = cur.fetchone()
	
	doc_id = metadata["id"]
	# make a list of all authors
	cur.execute("""SELECT authors.author_name 
						FROM authors, paper_authors 
						WHERE authors.id=paper_authors.author_id 
						AND paper_authors.paper_id=?""", [doc_id] )
	row_list = cur.fetchall()
	a_list = [ r["author_name"].encode('UTF-8') for r in row_list ]
	authors_str = ", ".join(a_list)
	
	if not metadata:	# paper id not found in matadata db
			content = '<p class="error"> paper id not found in matadata db </p>'
	else:
		
		content = ''		
		
		content += '\n<h2>' + metadata["title"].encode('UTF-8') + "</h2>\n"
		
		content += '\n<p class="paper_author_list">' + authors_str + '</p>'
		
		content += '\n<p class="paper_abstract"> <b>abstract:</b> ' + metadata["abstract"].encode('UTF-8') + "</p>"
		
		
		content += '\n<p> <h3 class="paper_metadata">Medatada:</h3></b> \n <dl>'
		

		for item in ['oai_identifier', 'categories',  'comments', \
					 'doi',  'arxiv_id',  'journal_ref',  'msc_class',  \
					 'report_no',  'created',  'updated']:
			if metadata[item]:
					content +='\n<dt>'+item+':</dt>'
					content +='\n<dd>'+metadata[item].encode('UTF-8')+'</dd>'
		content += '\n</dl>'

	content += '\n<p class="paper_arxiv_abstract"><a href="http://arxiv.org/abs/quant-ph/' + arxiv_num + '">Full article&nbsp;&#x25B8;</a></p>'
...
topics_per_row = 4 #make this variable?
...
def get_xml_topic_table(relations):#TODO: figure out better way to handle relations/db
    ...
    for topic_set in topic_sets:
        ...
        num_terms = 7 #TODO:set this elsewhere, perhaps a template file pref?
...
def get_doc_metadata(arxiv_num):
...
def get_xml_topic_docs_column(relation, topic):
    ...
    for doc in doc_keys[0:100]:
        arxiv_num = doc.title[0:7]
        doc_meta = get_doc_metadata(arxiv_num)  
        doc_column += '<tr class="list"><td onclick="window.location.href=\'../docs/' + doc.get_safe_title() + '.html\'">'+ doc_meta["title"].encode('UTF-8') + ' [' + doc.title +'] ' +'</td></tr>\n'
...
def get_xml_term_docs_column(relations, term):
    ...
    for doc in doc_keys[:20]:
        ...
        arxiv_num = doc.title[0:7]
        doc_meta = get_doc_metadata(arxiv_num)  
        doc_column += '<tr class="list doc_for_word"><td onclick="window.location.href=\'../docs/' + doc.get_safe_title() + '.html\'">' + doc_meta["title"].encode('UTF-8') + ' [' + doc.title +'] ' +'</td></tr>\n'
...
def get_xml_doc_docs_column(relation, parent_doc):
    ...
    doc_keys = docs.keys()
    doc_keys.sort(lambda x, y: cmp(docs[x], docs[y]))
    for doc in doc_keys[:25]:
        # strip only the number (no version info)
        arxiv_num = doc.title[0:7]
        doc_meta = get_doc_metadata(arxiv_num)    
        doc_column += '<tr class="list"><td title="' + doc_meta["title"].encode('UTF-8') + '" onclick="window.location.href=\'../docs/' + doc.get_safe_title() + '.html\'">' + doc.title + '</td></tr>\n'
...
def get_xml_doc_graph(relations):
    ...
    # TODO
    # docs_metadata = relations.get_docs_metadata()
    # 
    ...
    for doc in docs:
        ...
        docs_table += '<table width="100.0%"><tr><td class="dark-hover"><a title="' + doc_meta["title"].encode('UTF-8') +'" href="../docs/' + doc.get_safe_title() + '.html">' + doc.title + '</a></td></tr></table>\n'
        topics_table += '<table class="high-contrast-table" width="100.0%"><tr>\n'
...
def get_html_navbar():
    navbar = """<div id="navigation">\n<hr noshade>\n<table><tr>\n    <td class="navtext">Browse&nbsp;&#x25B8;</td> \n    <td class="linked"><a>Topics</a>\n      <ul> \n        <li><a href="../browse/topic-list.html">List of Terms</a></li> \n        <li><a href="../browse/topic-graph.html">Term Distributions</a></li> \n        <li><a href="../browse/topic-presence.html">Relative Presence</a></li>\n      </ul> \n    </td> \n    <td class="linked"><a>Documents</a>\n      <ul>\n        <li><a href="../browse/doc-graph.html">Topic Distributions</a></li>\n      </ul> \n    </td>\n    <td class="linked"><a>Terms</a>\n      <ul> \n        <li><a href="../browse/term-list.html">List by Frequency</a></li> \n        <li><a href="../browse/term-graph.html">Topic Distributions</a></li> \n       </ul> \n    </td>\n    <td class="linked"><a>About</a>\n      <ul>\n
            <li><a href="http://www.cs.princeton.edu/~blei/lda-c/">Blei LDA-C</a></li>
        <li><a href="http://www.ics.uci.edu/~newman/code/">Newman topicmodel</a></li>
        <li><a href="http://code.google.com/p/tmve/wiki/TMVE01">TMVE 0.1</a></li>
        <li><a href="http://nlp.fi.muni.cz/projekty/gensim/">gensim</a></li>
        <li><a href="https://github.com/ivanistheone/Latent-Dirichlet-Allocation">liblda</a></li>
